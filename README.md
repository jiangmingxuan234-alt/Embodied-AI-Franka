...
### Diffusion Policy（实验性，已归档）

基于 DDPM + Conditional Transformer（DiT 风格）的扩散策略。

| 参数 | 值 |
|------|-----|
| 噪声预测网络 | Conditional Transformer (4 层, d_model=256) |
| 观测历史 | 4 帧 |
| 预测长度 | 8 步 |
| 扩散步数 | 100 (DDPM) / 10 (DDIM 推理) |
| Epochs | 300 |

归档原因：策略能靠近物体但卡在悬停区（~7cm 高度），无法完成下压抓取。

### VLA (Vision-Language-Action)（实验性）

基于 ViT-B/16 + 语言编码器 + Transformer Decoder 的视觉-语言-动作模型。

| 参数 | 值 |
|------|-----|
| 视觉编码器 | ViT-B/16（ImageNet 预训练，最后 3 层解冻） |
| 语言编码器 | 可学习词嵌入 + MLP |
| 动作解码器 | Transformer Decoder (4 层) + Action Chunking (16 步) |
| 输入 | 128x128 RGB 图像 + 语言指令 + 37 维状态 |
| 可训练参数 | ~8M（ViT 后 3 层 + fusion + decoder） |
| Epochs | 300 |

实验结论：VLA 能引导机械臂靠近物体（dist 从 0.5 降到 0.08），action chunking + temporal ensemble 消除了震荡，但无法完成最后的下压抓取。悬停问题与 Diffusion Policy 一致，根因是训练数据中悬停帧占比过高。
...
| ACT (Temporal Ensemble) | 较高 | 高（chunk 加权平均） | 端到端，部署简洁，推荐方案 |
| Diffusion Policy (DiT) | 低 | 中 | 卡在悬停区，已归档 |
| PPO (纯 RL) | 低 | 低（逐帧决策） | 探索效率低，夹爪控制难学 |
| ACT → RL Fine-tune | ~12% | 中（需后处理） | 策略能靠近但抓取不稳定 |
| VLA (ViT + 语言) | 低 | 高（chunk 平滑） | 能靠近但下压失败，实验性 |

结论：对于精密抓取任务，模仿学习（BC/ACT）显著优于 RL 和端到端 VLA。ACT 的 temporal ensemble 是平滑控制的关键。VLA 的价值在于多任务泛化（换物体、换指令），但在单任务场景下不如专门训练的 ACT。所有方案共同的瓶颈是"悬停区"问题——训练数据中物体上方的帧占比过高，导致策略倾向于停留而非下压。
